...
//Ticket card component for each card
    ...
    <div key={id} className={styles["card"]}>
      <div className={styles["header"]}>
        ...
        <div style={{ position: "absolute", right: 0 }}>
          {display.grouping !== "userId" &&
            <UserCircle
              name={ticket.user.name}
              status={ticket.user.available ? "active" : "inactive"}
            />}
        </div>
      </div>
      ...
      <div className={styles["title"]}>
        {display.grouping !== "status" &&
          <div>
            {cardIcons.status[ticket.status]}
          </div>}
        <div>
          {ticket.title}
        </div>
      ...
      <div className={styles["featureSection"]}>
        {display.grouping !== "priority" &&
          <div className={styles["feature"]}>
            {cardIcons.priority[ticket.priority]}
          </div>}
        <div className={styles["feature"]}>
          {cardIcons.feature}
          <div>
